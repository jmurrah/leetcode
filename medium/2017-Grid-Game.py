...
At the start of the game, the first robot moves from (0, 0) to (1, n-1), collecting all the points from the cells on its path. For all cells (r, c) traversed 
on the path, grid[r][c] is set to 0. Then, the second robot moves from (0, 0) to (1, n-1), collecting the points on its path. Note that their paths may intersect 
with one another.
...
class Solution:
    def gridGame(self, grid: List[List[int]]) -> int:
        pre = 0
        post = sum(grid[0][i] for i in range(len(grid[0])))
        output = float("inf")

        for i in range(len(grid[0])):
            post -= grid[0][i]
            output = min(output, max(pre, post))
            pre += grid[1][i]
